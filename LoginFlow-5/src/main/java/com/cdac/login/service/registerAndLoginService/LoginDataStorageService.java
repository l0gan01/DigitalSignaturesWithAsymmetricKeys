...
@Service 
public class LoginDataStorageService{
	@Autowired
	...
	public void uploadFile(String invited, MultipartFile file, String invitee) {
		...
		try {
			ii = new InvitedInvitee(0,invitee, invited, file.getBytes(), false, null, file.getOriginalFilename(),localDateTime.toString());
		}
		catch(Exception e) {
	...
	public String getPassword(String user) {
		Optional<String> passObj = daoObj.getPasswordOfLoggingUser(user);
		return passObj.get();
	}
}
